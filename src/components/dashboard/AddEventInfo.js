...
const AddEventInfo = () => {
  ...
  const createEventInfo = async (e) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_BASEURL}/eventinfo`, formData, {
        headers: {
  ...
  const deleteEventInfo = async (eventInfoId) => {
    try {
      await axios.delete(`${process.env.REACT_APP_BASEURL}/eventinfo/${eventInfoId}`)
      getEventInfo()
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className='add-couple-info'>
      <p>{visible == "block" ? "Set Info Undangan" : "Info Udangan Sudah Diisi, Silahkan Edit Atau Delete"}</p>
      <form onSubmit={createEventInfo} style={{ display: visible }}>
        <div className="filed">
        ...
        <tbody>
          {eventInfo.map((info, i) => (
            <tr key={info.uuid}>
              ...
              <td>
                <button className='btn-edit'><Link to={`editeventinfo/${info.uuid}`}>Edit</Link></button>
                <button className='btn-edit' onClick={() => deleteEventInfo(info.uuid)}>Delete</button>
              </td>
            </tr>
